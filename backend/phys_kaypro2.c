/* Kaypro II / Kaypro 2 format, single-sided, 200k */
...
static void best_read_order(struct phys *this, struct sector_list *out, int track, int side) {
	struct sector_list sector_list[]={{0,0},{3,1536},{1,512},{4,2048},{2,1024},{8,4096},{6,3072},{9,4608},{7,3584},{5,2560}};

	memcpy(out,sector_list,sizeof(sector_list));
}

struct phys phys_kaypro2={
	.min_track = min_track,
	.max_track = max_track,
